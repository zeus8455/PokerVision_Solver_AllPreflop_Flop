...
## V2.53.0 вЂ” bridge-sourced solver payload fallback
...
Goal:
- Fix fallback-click execution gate after V2.51/V2.52.
- Prevent explicit bridge fallback decisions from being skipped when normal solver payload creation fails.
- Allow `active_invalid_hero_cards` and `postflop_solver_missing` bridge decisions to reach v11 runtime extraction and click planning.

Live symptom fixed:
- V2.52 created `active_invalid_hero_cards` with `target_sequence=["FOLD"]`.
- Runtime still produced `click.status=skipped`.
- Cause: `runtime/v11_stage1_runtime.py` called `build_and_save_solver_payload(...)` before extracting the injected bridge decision.
- If solver payload creation failed on invalid Clear_JSON, v11 returned `Solver payload was not created.` and never reached `_extract_solver_preflop_decision_from_state(...)`.

Changed:
- `external/PokerVisionFinalVersionNoSolver_snapshot/PokerVision V1_2/runtime/v11_stage1_runtime.py`
  - Added V2.53 bridge-sourced solver payload fallback wrapper around `build_and_save_solver_payload`.
  - If normal solver payload creation fails but `full_state["solver_preflop_bridge_contract"]` contains an explicit runtime fallback action decision:
    - builds a minimal `solver_payload_v2_53_bridge_fallback`
    - writes a diagnostic bridge payload JSON
    - continues to `_extract_solver_preflop_decision_from_state(...)`
  - Allows only explicit runtime-safe bridge fallbacks:
    - `active_invalid_hero_cards`
    - `postflop_solver_missing`
    - `safe_runtime_fallback`
  - Propagates `click_sequence` from `target_button_classes` / `target_sequence` into both action decision and bridge contract.
...
Validation:
- `tools/run_v2_53_bridge_payload_fallback_audit.py`
  - `V2.53_BRIDGE_PAYLOAD_FALLBACK_AUDIT_OK = True`
- `pytest tests/test_v2_53_bridge_payload_fallback_audit.py -q`
  - `1 passed`

Proof:
- Invalid HERO fallback:
  - payload status ok
  - payload source is bridge
  - payload file written
  - decision extracted
  - action is `fold`
  - click sequence is `["FOLD"]`
  - decision is not legacy `v12_stub_*`

- Postflop missing fallback:
  - payload status ok
  - payload source is bridge
  - payload file written
  - decision extracted
  - action is `check_fold`
  - click sequence is `["Check", "Check/fold", "FOLD"]`
  - decision is not legacy `v12_stub_*`

Notes:
- This does not change normal preflop solver payload construction.
- This only bypasses normal solver payload failure when a verified bridge fallback is already present in `full_state`.
- Full live validation should be repeated after this checkpoint.

## V2.54.0 вЂ” add static project logic map
...
Goal:
- Start Project Execution Map / Logic Coverage Auditor block.
- Add Layer A: static project map for the entire `C:\PokerVision_Solver_Preflop` tree.
- Inventory Python files, entrypoints, tests, tools, classes, functions, imports, runtime modules, solver modules, external snapshot modules, and likely legacy/active candidates.
...
Output:
- `outputs/v2_54_project_static_logic_map.json`
- `outputs/v2_54_project_static_logic_map.md`

Observed project inventory:
- Python files scanned: 217
...
- Runtime modules: 11
- Solver modules: 12
- External snapshot modules: 54
- Classes: 52
- Functions: 795
- Methods: 95
- Likely active candidates: 123
- Likely legacy/dead candidates: 125
- Syntax-error findings: 61

Important:
- Syntax errors are treated as audit findings, not as fatal tool failure.
- Many syntax-error findings are expected to be generated/temp/legacy/probe fragments and must be reviewed before cleanup.
- No files are deleted by this block.

Validation:
- `tools/run_v2_54_project_static_logic_map.py --skip-pytest-collect`
  - `V2.54_PROJECT_STATIC_LOGIC_MAP_OK = True`
- `pytest tests/test_v2_54_project_static_logic_map.py -q`
  - `1 passed`

Next:
- Layer B: dynamic execution map.
- Track which files/functions are actually executed by targeted regressions and live probes.
- Then merge static + dynamic reports before any cleanup/deletion.

## V2.55.0 — add dynamic project execution map
...
Goal:
- Add Project Execution Map / Logic Coverage Auditor Layer B.
- Track which project Python files/functions are actually executed by selected test/tool runs.
- Compare static project inventory against dynamic runtime/test coverage before cleanup.
...
Capabilities:
- Traces executed project Python files.
- Traces executed function calls and call counts.
- Reports not-executed Python files.
- Separates buckets:
  - solver_preflop
  - external snapshot runtime
  - external snapshot logic
  - tools
  - tests
- Reports suspicious legacy/not-executed candidates.
- Supports smoke and targeted/default profiles.

Important fix:
- Initial tracer was too slow under pytest because it resolved paths for every `sys.settrace` frame.
- Repaired with:
  - filename → relpath cache
  - fast non-project frame rejection
  - no `Path.resolve()` hot path
  - progress prints for pytest/tools

Validation:
- Smoke:
  - `V2.55_PROJECT_DYNAMIC_EXECUTION_MAP_OK = True`
  - `1 passed`
- Targeted pytest trace:
  - 6 selected tests passed
  - `V2.55_PROJECT_DYNAMIC_EXECUTION_MAP_OK = True`

Targeted dynamic result:
- commands_total: 1
- commands_ok: 1
- commands_failed: 0
- executed_files_total: 7
- not_executed_files_total: 212
- executed_functions_total: 13
- function_calls_total: 13
- suspicious_legacy_not_executed_total: 71

Notes:
- This is not full live/full project coverage yet.
- It proves the dynamic tracer works on selected current-chain tests.
- Cleanup/deletion must wait for Layer C merge report.

## V2.56.0 — add project logic coverage merge report
...
Goal:
- Add Project Execution Map / Logic Coverage Auditor Layer C.
- Merge static project map V2.54 and dynamic execution map V2.55.
- Produce a structured report before cleanup/deletion decisions.
...
Capabilities:
- Combines:
  - static Python file inventory
  - dynamic executed files/functions
  - git tracked files
  - optional pytest collect
- Reports:
  - active logic chain
  - tested logic chain
  - untested but referenced / likely active files
  - dead / legacy candidates
  - missing fixture dependencies
  - old tests needing rewrite candidates
  - runtime-only files
  - YOLO/model-dependent files
  - safe-to-delete candidates

Targeted merge result:
- static_python_files_total: 221
- dynamic_executed_files_total: 8
- dynamic_not_executed_files_total: 213
- git_tracked_files_total: 244
- active_logic_chain_total: 4
- tested_logic_chain_total: 8
- untested_but_referenced_total: 120
- dead_legacy_candidates_total: 122
- missing_fixture_dependencies_total: 287
- old_tests_needing_rewrite_total: 65
- runtime_only_files_total: 11
- yolo_model_dependent_files_total: 106
- safe_to_delete_candidates_total: 91

Validation:
- `V2.56_PROJECT_LOGIC_COVERAGE_REPORT_OK = True`
- `pytest tests/test_v2_56_project_logic_coverage_report.py -q`
  - `1 passed`

Important:
- `safe_to_delete_candidates` are candidates only.
- No deletion is performed by this block.
- Cleanup must be done in separate reviewed blocks.

## V2.57.0 — add cleanup review report
...
Goal:
- Add cleanup review layer after static/dynamic/merge coverage reports.
- Classify project files into safe review groups before any deletion.
...
Policy:
- No files are deleted by this block.
- `external/PokerVisionFinalVersionNoSolver_snapshot` is protected and not cleaned directly.
- DELETE_CANDIDATE still requires a separate reviewed deletion block and regression tests.

Observed:
- files_scanned: 6570
- keep_total: 1102
- review_total: 5361
- archive_candidate_total: 100
- delete_candidate_total: 7
- external_protected_total: 1049
- json_folders_total: 341

Validation:
- `V2.57_CLEANUP_REVIEW_REPORT_OK = True`
- `pytest tests/test_v2_57_cleanup_review_report.py -q`
  - `1 passed`

## V2.58.0 — add delete candidate inspection
...
Goal:
- Inspect V2.57 DELETE_CANDIDATE files before any deletion.
- Confirm exact file list, git tracked status, references, size, hash, preview and deletion verdict.

Added:
- `tools/run_v2_58_delete_candidate_inspection.py`
- `tests/test_v2_58_delete_candidate_inspection.py`

Result:
- delete_candidates_total: 10
- APPROVE_DELETE_CANDIDATE: 2
- BLOCK_DELETE_REVIEW: 7
- REVIEW_DELETE_CANDIDATE: 1
- ALREADY_MISSING: 0

Approved delete candidates:
- `apply_v2_58_delete_candidate_inspection.ps1`
- `apply_v2_58_delete_candidate_inspection_fast.ps1`

Finding:
- No solver_preflop source files were approved for deletion.
- No external snapshot files were approved for deletion.
- No core tools/tests were approved for deletion.
- Most candidates were generated `outputs/` reports, not project source.

Policy:
- V2.58 does not delete files.
- Deletion must be done in a separate reviewed cleanup block.
- First safe cleanup target is temporary apply scripts and generated outputs only.

Validation:
- `V2.58_DELETE_CANDIDATE_INSPECTION_OK = True`
- `pytest tests/test_v2_58_delete_candidate_inspection.py -q`
  - `1 passed`

## V2.59.0 — add fixture json trust review
...
Goal:
- Review JSON/fixture trust before any cleanup.
- Separate true fixtures from generated outputs and protected external snapshot JSON.

Added:
- `tools/run_v2_59_fixture_json_trust_review.py`
- `tests/test_v2_59_fixture_json_trust_review.py`

Observed:
- json_files_total: 6111
- json_folders_total: 340
- git_tracked_json_total: 17
- referenced_json_total: 16
- json_parse_ok_total: 6111
- json_parse_error_total: 0
- external_protected_json_total: 771
- fixture_truth_source_keep_total: 2
- current_referenced_keep_total: 14
- fixture_review_total: 6
- generated_output_delete_candidate_total: 5318

Policy:
- No JSON files are deleted by this block.
- external snapshot JSON is protected.
- generated outputs are not source-of-truth.
- next cleanup step should inspect the 6 FIXTURE_REVIEW JSON files.

Validation:
- `V2.59_FIXTURE_JSON_TRUST_REVIEW_OK = True`
- `pytest tests/test_v2_59_fixture_json_trust_review.py -q`
  - `1 passed`

## V2.60.0 — add fixture review inspection
...
Goal:
- Inspect the 6 V2.59 FIXTURE_REVIEW JSON files.
- Decide whether they are safe to delete, archive, keep, or migrate.

Added:
- `tools/run_v2_60_fixture_review_inspection.py`
- `tests/test_v2_60_fixture_review_inspection.py`

Observed:
- fixture_review_total: 6
- migrate_to_current_fixture_total: 6
- keep_total: 0
- archive_candidate_total: 0
- delete_candidate_total: 0

Finding:
- All 6 files are from `tests/fixtures/v2_40_real_clear_json_adapter/clear_json`.
- They are not approved for deletion.
- They should be migrated into current fixture coverage or kept as a dedicated real Clear_JSON adapter regression pack.

Validation:
- `V2.60_FIXTURE_REVIEW_INSPECTION_OK = True`
- `pytest tests/test_v2_60_fixture_review_inspection.py -q`
  - `1 passed`
