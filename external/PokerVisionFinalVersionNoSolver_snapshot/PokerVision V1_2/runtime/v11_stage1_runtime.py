...
# =============================================================================
# V2.53 BRIDGE-SOURCED SOLVER PAYLOAD FALLBACK
# =============================================================================

_V253_ORIGINAL_BUILD_AND_SAVE_SOLVER_PAYLOAD = build_and_save_solver_payload


def _v253_get_bridge_action_decision(full_state: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    contract = full_state.get("solver_preflop_bridge_contract")
    if not isinstance(contract, dict):
        return None

    payload = contract.get("bridge_payload")
    if not isinstance(payload, dict):
        return None

    action_decision = payload.get("action_decision")
    if not isinstance(action_decision, dict):
        return None

    context = action_decision.get("decision_context")
    if not isinstance(context, dict):
        context = {}

    if not bool(context.get("solver_preflop_runtime_source")):
        return None

    allowed = (
        bool(context.get("active_invalid_hero_cards"))
        or bool(context.get("postflop_solver_missing"))
        or bool(context.get("safe_runtime_fallback"))
    )
    if not allowed:
        return None

    target_classes = action_decision.get("target_button_classes")
    if not isinstance(target_classes, list) or not target_classes:
        target_classes = context.get("target_sequence")
    if not isinstance(target_classes, list) or not target_classes:
        return None

    # V2.53 repair: v11 _extract_solver_preflop_decision_from_state reads
    # click_sequence only from action_decision/contract. V2.51/V2.52 fallback
    # bridge contracts carried target_sequence/target_button_classes, so the
    # decision could be extracted but its click_sequence stayed empty.
    target_classes = [str(item) for item in target_classes if str(item).strip()]
    if not target_classes:
        return None
    action_decision["click_sequence"] = list(target_classes)
    action_decision.setdefault("target_button_classes", list(target_classes))
    contract["click_sequence"] = list(target_classes)
    contract.setdefault("target_sequence", list(target_classes))

    return action_decision


def _v253_build_bridge_solver_payload(
    *,
    full_state: Dict[str, Any],
    cycle_dir: Path,
    failure: Exception,
) -> Tuple[Dict[str, Any], Path]:
    action_decision = _v253_get_bridge_action_decision(full_state)
    if not isinstance(action_decision, dict):
        raise failure

    contract = full_state.get("solver_preflop_bridge_contract")
    table = full_state.get("table") if isinstance(full_state.get("table"), dict) else {}
    pipeline_meta = full_state.get("pipeline_meta") if isinstance(full_state.get("pipeline_meta"), dict) else {}

    table_id = str(table.get("table_id") or contract.get("table_id") or "unknown_table")
    hand_id = str(table.get("hand_id") or full_state.get("hand_id") or "unknown_hand")
    frame_name = str(
        table.get("frame_name")
        or action_decision.get("source_decision_frame_id")
        or pipeline_meta.get("frame_name")
        or "unknown_frame"
    )

    out_dir = Path(cycle_dir or ".")
    out_dir.mkdir(parents=True, exist_ok=True)
    payload_path = out_dir / f"{table_id}_{frame_name}_v253_bridge_solver_payload.json"

    payload = {
        "schema_version": "solver_payload_v2_53_bridge_fallback",
        "status": "ok",
        "source": "solver_preflop_bridge_contract",
        "reason": "v253_bridge_payload_built_after_solver_payload_failure",
        "original_payload_failure": str(failure),
        "table_id": table_id,
        "hand_id": hand_id,
        "frame_name": frame_name,
        ...
        "target_button_classes": action_decision.get("target_button_classes"),
        "runtime_safe_fallback": True,
    }

    try:
        payload_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")
    except Exception:
        payload_path = out_dir / "v253_bridge_solver_payload_unwritten.json"

    return payload, payload_path


def build_and_save_solver_payload(full_state: Dict[str, Any], *, cycle_dir: Path) -> Tuple[Dict[str, Any], Path]:  # type: ignore[no-redef]
    try:
        return _V253_ORIGINAL_BUILD_AND_SAVE_SOLVER_PAYLOAD(full_state, cycle_dir=cycle_dir)
    except Exception as exc:
        return _v253_build_bridge_solver_payload(
            full_state=full_state,
            cycle_dir=cycle_dir,
            failure=exc,
        )
